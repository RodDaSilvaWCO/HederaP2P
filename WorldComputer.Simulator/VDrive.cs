...
namespace WorldComputer.Simulator
{
    public class VDrive
    {
        ...
        private static string mWorldComputerVirturalDiskID = "00000000000000000000000000000000";
        private static string mWorldComputerVirtualDiskVolumeID = null!;
        ...
        public static string Init(VDriveInfo vdriveinfo, string clientMountRoot , string clientMountDataDirectoryTemplate )
        {
            ...
            if (HandleParameters(args) == 0)
            {
                ...
                if (mOptMountingPoint != "" && mRootPath != "")
                {
                    ...
                    if(mountFailed)
                    {
                        Console.WriteLine("*** ERROR:  This was an issue mounting the virtual disk.  Please try again.");
                    }
                    op = true;
                }
            }
        ...
        static int RunMounting()
        {
            ...
            if (mCBFSConn != null)
            {
                ...
                Console.WriteLine($"");
                Console.WriteLine($"VDisk Stoage Array: ({clusters.Length},{numReplicas})");
                Console.WriteLine($"");
                ...
                for(int c = 0; c < numClusters; c++)
                {
                    ...
                    for ( int r = 0; r < numReplicas; r++)
                    {
                        ...
                        Console.Write($"{ Program.ComputerNodeNumberFromPort(int.Parse(node[1])).ToString().PadLeft(3)}");
                        if (r < numReplicas - 1) Console.Write(", ");
                    }
                    Console.WriteLine($"   ]");
                }
        ...
        static void Usage()
        {
            //Console.WriteLine("Usage: WCVDrive.exe [-<switch 1> ... -<switch N>] <mapped folder> <mounting point>");
            Console.WriteLine("Usage: WCVDrive.exe <vDisk ID> <mapped folder> <mounting point>");
            Console.WriteLine("");
            Console.WriteLine("For example:  WCVDrive.exe C:\\temp Z:");
        ...
        internal static string WCVirtualDiskMountApi( string vdiskID )  // NOTE:  Also called by Cluster /CREATE
        {
            ...
            {
                Console.WriteLine($"Error calling VirtualDiskMount() - response.StatusCode={response.StatusCode}");
            }
            return mountResult;
        }
        ...
        internal static void WCVirtualDiskUnmountApi() // NOTE:  Also called by Cluster /CREATE
        {
            ...
            if (response.StatusCode != HttpStatusCode.OK)
            {
                Console.WriteLine($"Error calling VirtualDiskUnmount() - response.StatusCode={response.StatusCode}");
            }
            else
            {
        ...
        private static string WCVirtualDiskVolumeMetaDataOperationApi( string base64Operation )
        {
            ...
            {
                Console.WriteLine($"Error calling WCVirtualDiskVolumeMetaDataOperationApi() - response.StatusCode={response.StatusCode}");
            }
            return result;
        }


        private static string WCVirtualDiskVolumeDataOperationApi(string base64Operation)
        {
            string result = null!;
            var json = @$"
            {{
            ...
            {
                Console.WriteLine($"Error calling WCVirtualDiskVolumeDataOperationApi() - response.StatusCode={response.StatusCode}");
            }
            return result;
        }
        #endregion 
    ...
    */
    ...
    public class VDriveInfo
    {
        ...
        //[JsonPropertyName("VDEF")]
        //public string VDriveDef { get; set; }   
